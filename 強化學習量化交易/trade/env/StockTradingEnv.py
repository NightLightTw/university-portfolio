...
class StockTradingEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if self.currentStep > len(self.df.loc[:, 'Open'].values) - 6: #防止超出data範圍
            self.currentStep = 0

        delayModifier = (self.currentStep / MAX_STEPS) #MAX_STEP=20000

        done = self.balance <= 0 or self.balance > MAX_ACCOUNT_BALANCE or (self.currentStep > len(self.df.loc[:, 'Open'].values) - 6)
    ...
    def reset(self):
        ...
        self.balance = INITIAL_ACCOUNT_BALANCE #10000
        ...
        self.currentStep = random.randint(0, len(self.df.loc[:, 'Open'].values) - 6) #隨機調整current

        return self._nextObservation()
    

    def render(self, mode='human', close=False, filename='render.txt'):
...
#         file = open(filename, 'a+')
#         file.write(f'Step: {self.currentStep}\n')
...
#             f'Avg cost for held shares: {self.averageShareCost} (Total sales value: {self.totalSalesValue})\n')
#         file.write(
#             f'Net worth: {netWorth} (Max net worth: {self.maxNetWorth})\n')
